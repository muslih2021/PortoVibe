    ...
    <div style={{ padding: '8rem 2rem', maxWidth: '1200px', margin: '0 auto', minHeight: '100vh', background: 'var(--bg)', transition: 'all 0.3s ease' }}>
      ...
      {portfolios.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '2rem' }}>
          {portfolios.map((p) => (
            <Link key={p.id} to={`/${p.id}`} target="_blank" className="benefit-card glow-card" style={{ textDecoration: 'none', color: 'inherit', textAlign: 'left', borderRadius: '24px', transition: 'all 0.3s ease' }}>
              <div style={{ height: '200px', background: 'var(--section-bg)', borderRadius: '16px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem' }}>
                👤
              </div>
